...
function timersAndStuff()
	-- intro
	if time() - eventTimer >= 0.0000 - OFFSET and eventCount == 0 then eventCount = 0.5 changeFlickSize(3 * ROTATION_FLICK_MULT) staticRotation = INTRO_ROTATION s_setStyle("FSoRintro") executeFadeIn(13.151)
	elseif time() - eventTimer >= 0.3100 - OFFSET and eventCount == 0.5 then eventCount = 0.7 patternsEnabled = true 
	elseif time() - eventTimer >= 0.3200 - OFFSET and eventCount == 0.7 then eventCount = 1 allowPreDelay = 1
	...
	elseif time() - eventTimer >= 42.740 - OFFSET and eventCount == 4.5 then eventCount = 5 enableFlick(8) thickness = 40 l_setSpeedMult(l_getSpeedMult() + SPEED_DROP_INCR) l_setWallSkewRight(7)
	elseif time() - eventTimer >= 44.384 - OFFSET and eventCount == 5 then eventCount = 5.5 enableFlick(0) s_setStyle("FSoRblack") addAdditionalBgPatterns(0)
