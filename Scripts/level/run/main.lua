...
function pRandomBarrageLR(mTimes, mStep, delay)
    delay = delay or getPerfectDelay(THICKNESS) * 7
...
function pLRBarrage(mTimes)
    ...
    t_wait(getPerfectDelay(THICKNESS) * 7 * 0.85)
end

function pLRBarrage2(mTimes, delay)
    local side = math.random(0, l_getSides())
    delay = delay or getPerfectDelay(THICKNESS) * 6
...
function addPattern(mKey)
	...
	elseif mKey == 1 then pLRBarrage(math.random(1, 2))
