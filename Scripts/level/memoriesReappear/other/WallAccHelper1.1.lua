...
function wallDist(side, thickness, speedmult, dist, minspeed)
	w_wallAcc(side, thickness, speedmult/l_getSpeedMult(), -1/60*(speedmult/l_getSpeedMult())^2/dist, minspeed/l_getSpeedMult(), speedmult/l_getSpeedMult())
end
...
function loadBMPPicture(path, xos, yos)
	...
	for y = 0, height-1 do
		for x = 0, width-1 do
			pixel = readHex(3)
				if pixel == 0 then table.insert(grid, {(x-width/2)*width/height + xos*width/2, y-height/2 + yos*height/2})
			elseif pixel <= 0xff then for i = 1, pixel do table.insert(grid, {(x-width/2)*width/height + xos*width/2, y-height/2 + yos*height/2}) end
			end
		end
