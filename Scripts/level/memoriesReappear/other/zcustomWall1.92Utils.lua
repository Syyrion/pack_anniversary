...
conv_wall_module = {
	...
	wallAcc = function(self, side, thickness, speedAdj, acceleration, minSpeed, maxSpeed, shape, speed, distance)
		...
		self:_wall(side, shape, thickness, distance, speedAdj * ((speed or l_getSpeedMult()) * u_getDifficultyMult() ^ 0.65), acceleration, minSpeed * ((speed or l_getSpeedMult()) * u_getDifficultyMult() ^ 0.65), maxSpeed * ((speed or l_getSpeedMult()) * u_getDifficultyMult() ^ 0.65))
	end,
	wallAdj = function(self, side, thickness, speedAdj, shape, speed, distance)
		self = conv_wall_module.find_self(self)
		self:_wall(side, shape, thickness, distance, speedAdj * ((speed or l_getSpeedMult()) * u_getDifficultyMult() ^ 0.65), 0, 0, 0)
	end,
	...
	_wall = function(self, side, shape, thickness, distance, speed, acceleration, minSpeed, maxSpeed)
		...
		cw_setVertexPos(wall.cw, 2, self._getOrbit(angle + div * 0.5 + l_getWallAngleLeft(), (distance or self.WALL_SPAWN_DIST) + thickness + l_getWallSkewLeft()))
		cw_setVertexPos(wall.cw, 3, self._getOrbit(angle - div * 0.5 + l_getWallAngleRight(), (distance or self.WALL_SPAWN_DIST) + thickness + l_getWallSkewRight()))
...
function v192WallAcc(mSide, mThickness, mSpeedAdj, mAcceleration, mMinSpeed, mMaxSpeed, mShape, mSpeed, mDistance)
	conv_wall_module.wallAcc(nil, mSide, mThickness, mSpeedAdj, mAcceleration, mMinSpeed, mMaxSpeed, mShape, mSpeed, mDistance)
end
...
function tv192WallAdj(mSide, mThickness, mSpeedAdj, mShape, mSpeed)
	t_eval(([[conv_wall_module.wallAdj(nil, %d, %d, %d, %d, %d, %d)]]):format(mSide, mThickness, mSpeedAdj, mShape, mSpeed, mDistance))
end

function tv192WallAcc(mSide, mThickness, mSpeedAdj, mAcceleration, mMinSpeed, mMaxSpeed, mShape, mSpeed)
	t_eval(([[conv_wall_module.wallAcc(nil, %d, %d, %d, %d, %d, %d, %d, %d, %d)]]):format(mSide, mThickness, mSpeedAdj, mAcceleration, mMinSpeed, mMaxSpeed, mShape, mSpeed, mDistance))
end
