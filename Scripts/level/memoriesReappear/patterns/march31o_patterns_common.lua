...
function pMarch31osBarrage(_side, _thickness, _neighbors, _sizeMult, _skipEndDelay, _isRebootingSide, _endAdditionalDelay
                           , _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(3, 5));
    _sizeMult = anythingButNil(_sizeMult, 1);
    _neighbors = anythingButNil(_neighbors, 0);
...
function pMarch31osRandomBarrage(_side, _thickness, _iter, _distMin, _distMax, _isRepeat, _isDelayDistance, _delMult,
                                 _sizeMult, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult,
                                 _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(2, 5));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _distMin = anythingButNil(_distMin, -getHalfSides("floor"));
    _distMax = anythingButNil(_distMax, getHalfSides("floor"));
    ...
    for a = 0, _iter, 1 do
        ...
        if getBooleanNumber(_isDelayDistance) then
            ...
            t_applyPatDel(customizePatternDelay((2 + (_barrageDistanceDelay * 2) * (6 / getProtocolSides())) *
                _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()))
        else t_applyPatDel(customizePatternDelay((5 * ((getProtocolSides() / 9) + 0.75)) * _curDelaySpeed * _delMult *
...
function pMarch31osAlternatingBarrage(_side, _thickness, _iter, _isSpiral, _hasContainedEnd, _gapLength,
                                      _repeatBarrageAmount, _repeatBarrageDelay, _isRepeatBarrageDelaySpd, _delMult,
                                      _sizeMult, _direction, _skipEndDelay, _isRebootingSide, _endAdditionalDelay,
                                      _addMult, _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(3, 5));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _hasContainedEnd = anythingButNil(_hasContainedEnd, 0);
    _gapLength = closeValue(isValueType(_gapLength, "number", false, _gapLength, 0), 0,
        math.floor(getProtocolSides() / 2) - 1);
    ...
    _repeatBarrageDelay = anythingButNil(_repeatBarrageDelay, 2);
    _isRepeatBarrageDelaySpd = anythingButNil(_isRepeatBarrageDelaySpd, 0);
    ...
    local _curDelaySpeed = (_addMult or march31oPatDel_AddMult or 1) -
        (getSpeedDelay(PAT_START_SPEED or u_getSpeedMultDM()) * (march31oPatDel_SDMult or 0));
    local _curSide = _side or u_rndInt(0, getProtocolSides() - 1);
    if _curSide == TARGET_PATTERN_SIDE and getBooleanNumber(_isRebootingSideStat) then _curSide = _curSide +
        getRandomDir() end
    TARGET_PATTERN_SIDE = (_isRebootingSideStat) and _curSide or -256;
    ...
    for a = 0, _iter, 1 do
        ...
        for repeatAmount = 0, _repeatBarrageAmount, 1 do
            cWallExM(_curSide + _barrageOffset,
                (math.floor(getProtocolSides() / 2) - _gapLength) + (_barrageDir * 0.5 - 0.5), 2,
                p_getPatternThickness() * _sizeMult);
            if _barrageLoopDir > 0 and _gapLength > 0 then
                for i = getProtocolSides() - (_gapLength * 2) - (getProtocolSides() % 2), getProtocolSides(), 1 do cWall(_curSide
                    + i + (_direction * 0.5 - 0.5), p_getPatternThickness() * _sizeMult); end
            end
            if repeatAmount < _repeatBarrageAmount then t_applyPatDel(customizePatternDelay(_repeatBarrageDelay *
                _sizeMult, _isRepeatBarrageDelaySpd)); end
        end
    ...
    if (_hasContainedEnd) then
        for repeatAmount = 0, _repeatBarrageAmount, 1 do
            ...
            elseif getProtocolSides() == 5 then for i = 0, getProtocolSides() - 3 do cWall(_curSide + i + _barrageOffset
                + closeValue(_barrageLoopDir, 0, 1)); end
            elseif getProtocolSides() > 5 then cBarrageDoubleHoled(_curSide + _barrageOffset +
                (math.floor(_gapLength / 2) * 2), 0, 0, p_getPatternThickness() * _sizeMult);
            end
            if repeatAmount < _repeatBarrageAmount then t_applyPatDel(customizePatternDelay(_repeatBarrageDelay *
                _sizeMult, _isRepeatBarrageDelaySpd)); end
        end
...
function pMarch31osVortaBarrage(_side, _thickness, _iter, _dirType, _revFreq, _dirPoints, _vortaBarType, _free, _delMult
                                , _sizeMult, _direction, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult,
                                _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    -- optional args
    _iter = anythingButNil(_iter, 4);
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _dirType = anythingButNil(_dirType, 0);
    _revFreq = anythingButNil(_revFreq, 1);
    _dirPoints = anythingButNil(_dirPoints,
        (_dirType == 2 and { u_rndInt(2, 5), u_rndInt(2, 5) } or { math.floor(_iter / 2) }));
    _vortaBarType = anythingButNil(_vortaBarType, 0);
    ...
    local _vortaBarPart = function(_side, _free, _thickness, _vortaBarType)
        ...
        if _vortaBarType == 1 then
            ...
            cWallEx(_side + (closeValue(getProtocolSides() % 3, 0, 1) + 3),
                closeValue((getProtocolSides() % 3) - 1, 0, 1) + 1, _thickness);
            if getProtocolSides() >= 9 then
                for i = 0, math.floor(getProtocolSides() / 3) - 3, 1 do cWallEx(_side + (i * 3) +
                    (closeValue(getProtocolSides() % 3, 0, 1) + closeValue((getProtocolSides() % 3) - 1, 0, 1) + 6), 1,
                    _thickness); end
            end
...
function pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, _holes, _offsetType, _offsetDistance, _mainLoopDir,
                                   _isInvertAccurate, _delMult, _sizeMult, _skipEndDelay, _isRebootingSide,
                                   _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(3, 5));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    ...
    for a = 0, _iter, 1 do
        ...
        if a % 2 == 1 and (l_getSides() % 2 == 1) and _holes == 1 and _offsetType == 2 and
            getBooleanNumber(_isInvertAccurate) then cWallEx(_curSide + 1 + getHalfSides(),
            getProtocolSides() - (2 + _gap), p_getPatternThickness());
        else
            if _holes > 1 then cBarrageExHoles(_curSide +
                ((a % _typeModeModuloStat) * _typeModeMultStat * _typeModeStat * _barrageLoopDir), _holes - 1,
                p_getPatternThickness());
            else cBarrageGap(_curSide + ((a % _typeModeModuloStat) * _typeModeMultStat * _typeModeStat * _barrageLoopDir
                ), _gap, p_getPatternThickness());
            end
        end
...
function pMarch31osBarrageSpiral(_side, _thickness, _iter, _gap, _distance, _isDelayDistance, _delMult, _sizeMult,
                                 _direction, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult,
                                 _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, 1, 0, anythingButNil(_distance, 1), _direction, false,
        (getBooleanNumber(_isDelayDistance) and (anythingButNil(_distance, 1) * 0.333 + 0.667) or 1) * _delMult,
        _sizeMult, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan,
...
--        _distance: the distance of this barrage
-- _isDelayDistance: is distance has an accurate delay in it?
function pMarch31osBarrageSpiralRev(_side, _thickness, _iter, _revFreq, _gap, _holes, _distance, _isDelayDistance,
                                    _delMult, _sizeMult, _direction, _skipEndDelay, _isRebootingSide, _endAdditionalDelay
                                    , _addMult, _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(2, 3));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _revFreq = anythingButNil(_revFreq, u_rndInt(1, 2));
    _distance = anythingButNil(_distance, 1);
    if type(_gap) ~= "number" or _gap < 1 then _gap = 1; end
    if type(_holes) ~= "number" or _holes < 1 then _holes = 1; end
    _isTight = anythingButNil(_isTight, 0);
    _skipEndDelay = anythingButNil(_skipEndDelay, 0);
    ...
    if _curSide == TARGET_PATTERN_SIDE and getBooleanNumber(_isRebootingSideStat) then _curSide = _curSide +
        getRandomNegVal(getRebootPatternSide()) end
    TARGET_PATTERN_SIDE = (_isRebootingSideStat) and _curSide or -256;
    local _barrageLoopDir = (type(_direction) == "number" and getNeg(_direction)) or (_direction == 0 and getRandomDir()
        ) or getRandomDir();
    ...
    for r = 0, _revFreq, 1 do
        ...
        for a = 0, _iter + _revIterAdd, 1 do
            ...
            if _holes > 1 then cBarrageExHoles(_curSide + (_barrageOffset * _distance), _holes - 1,
                p_getPatternThickness());
            else cBarrageGap(_curSide + (_barrageOffset * _distance), _gap, p_getPatternThickness());
            end
            _barrageOffset = _barrageOffset + _barrageLoopDir;
            t_applyPatDel(customizePatternDelay(4 *
                (getBooleanNumber(_isDelayDistance) and (_distance * 0.333 + 0.667) or 1) * _delMult * _sizeMult *
                _curDelaySpeed, p_getDelayPatternBool()));
        end
...
function pMarch31osBarrageReversals(_side, _thickness, _iter, _gap, _isInvertAccurate, _delMult, _sizeMult, _direction,
                                    ...
                                    _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, 1, 2, 1, _direction, _isInvertAccurate, _delMult * 2,
        _sizeMult, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan,
        _spdIsGreaterThanEqual)
end
...
function pMarch31osWallStrip(_side, _thickness, _repetitions, _mirrorStep, _extraWidth, _delMult, _sizeMult, _isThickness
                             , _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan,
                             _spdIsGreaterThanEqual)
    _repetitions = anythingButNil(_repetitions, 1);
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _mirrorStep = anythingButNil(_mirrorStep, u_rndInt(2, 3));
    _extraWidth = anythingButNil(_extraWidth, 0);
    _isTight = anythingButNil(_isTight, 0);
    _skipEndDelay = anythingButNil(_skipEndDelay, 0);
    ...
    if not _isThickness then
        ...
        cWallMirrorEx(_curSide, _mirrorStep, _extraWidth,
            customizePatternThickness((2.75 * _repetitions) * _delMult * _sizeMult * _curDelaySpeed, false) +
            (p_getPatternThickness() * _sizeMult));
...
function pMarch31osWhirlwind(_side, _iter, _extraWidth, _mirrorStep, _posSpacing, _seamless, _delMult, _sizeMult,
                             _direction, _slopeAmountStart, _slopeAmountEnd, _is_full, _skipEndDelay, _isRebootingSide,
                             _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(3, 6));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _extraWidth = anythingButNil(_extraWidth, 0);
    _mirrorStep = anythingButNil(_mirrorStep, math.floor(getProtocolSides() / 3));
    _posSpacing = anythingButNil(_posSpacing, 1);
    ...
    local currentCleanTimesEndSectionOfThickAmountForGreaterThanSquare = 3 * (_seamless and 1.1 or 1) *
        (
    ...
    if getProtocolSides() == 3 and ((_slopeAmountStart > 0 and _slopeAmountEnd > 0) and getBooleanNumber(_is_full)) then
        ...
        cBarrage(_curSide + _spiralPosistionOffset + _direction,
    ...
    elseif getProtocolSides() == 4 and ((_slopeAmountStart > 0 and _slopeAmountEnd > 0) and _is_full) then
        ...
        _spiralPosistionOffset = _spiralPosistionOffset + _direction;
        for i = 0, 1, 1 do cWall(_curSide + _spiralPosistionOffset + i * _direction,
        ...
        for a = 0, _iter + 1, 1 do --1, _iter + 2
            ...
            cWall(_curSide + _spiralPosistionOffset,
                customizePatternThickness(currentTimesOfThickAmountForSquare * currentSizeOverride * _sizeMult,
                    p_getDelayPatternBool()));
            _spiralPosistionOffset = _spiralPosistionOffset + _direction;
            t_applyPatDel(customizePatternDelay(4 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
        end
        _spiralPosistionOffset = _spiralPosistionOffset + _direction * 2;
        for k = 0, 1, 1 do cWall(_curSide + _spiralPosistionOffset + k * _direction,
        ...
        _spiralPosistionOffset = _spiralPosistionOffset + _direction * 3;
        ...
        if (_slopeAmountStart > 0) then
            for fa = 0, _slopeAmountStart, 1 do
                for ia = 0, _slopeAmountStart - fa, 1 do
                    if ia > 0 then cWallMirrorEx(_curSide - (_direction * _posSpacing * (ia - 1)), _mirrorStep,
                        _extraWidth,
                        customizePatternThickness((_curSpiralThick + 1) * _delMult * _sizeMult, p_getDelayPatternBool())); end
                end
                cWallMirrorEx(_curSide + (_direction * _posSpacing * fa) - (_slopeAmountStart * _direction * _posSpacing
                    ), _mirrorStep, _extraWidth,
                ...
                if fa == _slopeAmountStart then _spiralPosistionOffset = _spiralPosistionOffset +
                    (_direction * _posSpacing); end
            end
        end
        for a = 0, _iter, 1 do
            cWallMirrorEx(_curSide + _spiralPosistionOffset, _mirrorStep, _extraWidth,
        ...
        if (_slopeAmountEnd > 0) then
            for fb = 0, _slopeAmountEnd, 1 do
                ...
                for ib = 0, fb, 1 do cWallMirrorEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) +
                    (_direction * _posSpacing * ib), _mirrorStep, _extraWidth,
...
--                   _slopeAmountEnd: amount of slope after spiral pattern spawns
-- [WORKS ONLY ON SIDE < 5] _is_full: boolean of full spiral
function pMarch31osExWhirlwind(_side, _iter, _timesBeforeChangeDir, _revFreq, _extraWidth, _mirrorStep, _posSpacing,
                               _seamless, _delMult, _sizeMult, _direction, _slopeAmountStart, _slopeAmountCycle,
                               _slopeAmountEnd, _is_full, _skipEndDelay, _isRebootingSide, _endAdditionalDelay, _addMult
                               , _delayMultOfSpdLessThan, _spdIsGreaterThanEqual)
    _iter = anythingButNil(_iter, u_rndInt(4, 7));
    _delMult = anythingButNil(_delMult, 1);
    _sizeMult = anythingButNil(_sizeMult, 1);
    _extraWidth = anythingButNil(_extraWidth, 0);
    _mirrorStep = anythingButNil(_mirrorStep, math.floor(getProtocolSides() / 3));
    _posSpacing = anythingButNil(_posSpacing, 1);
    _is_full = anythingButNil(_is_full, 0);
    _timesBeforeChangeDir = closeValue((type(_timesBeforeChangeDir) == "number" and _timesBeforeChangeDir or 0), 0, 999,
        "min");
    ...
    local currentTimesOfDelayAmountForSquare, currentTimesOfThickAmountForSquare, currentTimesOfThickAmountBeforeDirChangeForSquare, currentThickOffsetAmountBeforeDirChangeForSquare_001, currentThickOffsetAmountBeforeDirChangeForSquare_002 = 2
        , 4, 6, 0, 0;
    local currentSizeOverride, currentDelayOverride = 1, 0.9;

    if getProtocolSides() == 3 and
        ((_slopeAmountStart > 0 and _slopeAmountCycle > 0 and _slopeAmountEnd > 0) and getBooleanNumber(_is_full)) then
        cBarrage(_curSide + _direction, customizePatternThickness(1 * _sizeMult, p_getDelayPatternBool()));
        for rev = 0, _revFreq, 1 do
            for a = 0, _iter, 1 do
                if a == _iter + 1 and rev == _revFreq then currentTimesOfDelayAmountForTriangle = 3; end
                if a == _iter and rev < _revFreq then currentTimesOfThickAmountForTriangle = 4 +
                    (_curDelaySpeed * _delMult); end
                cWall(_curSide + _spiralPosistionOffset,
                    customizePatternThickness(currentTimesOfThickAmountForTriangle * _sizeMult, p_getDelayPatternBool()));
                _spiralPosistionOffset = _spiralPosistionOffset + _direction;
                t_applyPatDel(customizePatternDelay(currentTimesOfDelayAmountForTriangle * _curDelaySpeed * _delMult *
                    _sizeMult, p_getDelayPatternBool()));
            end
            if rev < _revFreq then
                cWall(_curSide + _spiralPosistionOffset - _direction,
                    customizePatternThickness(5 * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                _direction = _direction * -1;
            end
        end
        cBarrage(_curSide + _spiralPosistionOffset + _direction,
            customizePatternThickness(1 * _sizeMult, p_getDelayPatternBool()));
    elseif getProtocolSides() == 4 and
        ((_slopeAmountStart > 0 and _slopeAmountCycle > 0 and _slopeAmountEnd > 0) and _is_full) then
        if _revFreq > 0 then
            _direction = _direction * -1;
            ...
            cBarrage(_curSide - _direction,
                customizePatternThickness(1 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
            for a = 0, _iter, 1 do
                if a == 1 then currentThickOffsetAmountBeforeDirChangeForSquare_002 = 0; end
                if a == _iter then currentTimesOfThickAmountForSquare = 11;
                currentTimesOfDelayAmountForSquare = 5;
                end
                cWall(_curSide,
                    customizePatternThickness((
                        currentTimesOfThickAmountForSquare + currentThickOffsetAmountBeforeDirChangeForSquare_001 +
                            currentThickOffsetAmountBeforeDirChangeForSquare_002) * currentSizeOverride * _sizeMult,
                        p_getDelayPatternBool()));
                if a < _iter then _curSide = _curSide - _direction; end
                t_applyPatDel(customizePatternDelay((
                    currentTimesOfDelayAmountForSquare + currentThickOffsetAmountBeforeDirChangeForSquare_001 +
                        currentThickOffsetAmountBeforeDirChangeForSquare_002) * currentSizeOverride * _sizeMult,
                    p_getDelayPatternBool()));
            end
            for rev = 0, _revFreq - 1, 1 do
                cWall(_curSide - _direction,
                    customizePatternThickness((
                        (_iter > 0 and 4 or 6) - currentThickOffsetAmountBeforeDirChangeForSquare_001) *
                        currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
                cBarrage(_curSide + _direction,
                    customizePatternThickness(1 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
                t_applyPatDel(customizePatternDelay(5 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
                for a = 0, _iter, 1 do
                    currentTimesOfThickAmountForSquare = 4;
                    currentTimesOfDelayAmountForSquare = 2;
                    if a == _iter then currentTimesOfThickAmountForSquare = 6;
                    currentTimesOfDelayAmountForSquare = 5;
                    end
                    cWall(_curSide + _direction,
                        customizePatternThickness(((a == _iter and (rev < _revFreq - 1 and 11 or 6) or 4)) *
                            currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
                    t_applyPatDel(customizePatternDelay((a == _iter and 5 or 2) * currentSizeOverride * _sizeMult,
                        p_getDelayPatternBool()));
                    _curSide = _curSide + _direction;
                end
            ...
            for a = 0, _iter + 1, 1 do
                if a == _iter + 1 then currentTimesOfThickAmountForSquare = 6; end
                cWall(_curSide + _spiralPosistionOffset,
                    customizePatternThickness((a == _iter + 1 and 7 or 9) * currentSizeOverride * _sizeMult,
                        p_getDelayPatternBool()));
                _spiralPosistionOffset = _spiralPosistionOffset + _direction;
                t_applyPatDel(customizePatternDelay(4 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
            end
        ...
        if (_slopeAmountStart > 0) then
            for fa = 0, _slopeAmountStart, 1 do
                for ia = 0, _slopeAmountStart - fa, 1 do
                    if ia > 0 then cWallMirrorEx(_curSide - (_direction * _posSpacing * (ia - 1)), _mirrorStep,
                        _extraWidth,
                        customizePatternThickness((_curSpiralThick + 1) * _sizeMult, p_getDelayPatternBool())); end
                end
                cWallMirrorEx(_curSide + (_direction * _posSpacing * fa) - (_slopeAmountStart * _direction * _posSpacing
                    ), _mirrorStep, _extraWidth,
                    customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
        ...
        for rev = 0, _revFreq, 1 do
            ...
            for a = 0, _iter - halt, 1 do
                cWallMirrorEx(_curSide + _spiralPosistionOffset, _mirrorStep, _extraWidth,
                    customizePatternThickness((
                        _curSpiralThick + (a == _iter - halt and (_slopeAmountCycle > 0) and rev < _revFreq and 1 or 0))
                        * _sizeMult, p_getDelayPatternBool()));
                ...
                if a == _iter - halt and rev == _revFreq then _spiralPosistionOffset = _spiralPosistionOffset +
                    (_direction * _posSpacing); end
            end
            if rev < _revFreq then
                if (_slopeAmountCycle > 0) then
                    for fb = 0, _slopeAmountCycle - 1, 1 do
                        for ib = 0, fb, 1 do cWallMirrorEx(_curSide +
                            (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib),
                            _mirrorStep, _extraWidth,
                            customizePatternThickness((_curSpiralThick + 1) * _sizeMult, p_getDelayPatternBool())); end
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult *
                            _sizeMult, p_getDelayPatternBool()));
                    end
                    for hb = 0, _timesBeforeChangeDir, 1 do
                        if hb < _timesBeforeChangeDir then
                            for ib = 0, _slopeAmountCycle, 1 do cWallMirrorEx(_curSide +
                                (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib),
                                _mirrorStep, _extraWidth,
                                customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult * _sizeMult,
                                    p_getDelayPatternBool())); end
                        elseif hb == _timesBeforeChangeDir then
                            for ib = 0, _slopeAmountCycle - 1, 1 do cWallMirrorEx(_curSide +
                                (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib),
                                _mirrorStep, _extraWidth,
                                customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult * _sizeMult,
                                    p_getDelayPatternBool())); end
                            cWallMirrorEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) +
                                (_direction * _posSpacing * _slopeAmountCycle), _mirrorStep, _extraWidth,
                                customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
                        end
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult *
                            _sizeMult, p_getDelayPatternBool()));
                    ...
                    for vb = 1, _slopeAmountCycle, 1 do
                        for eb = 0, _slopeAmountCycle - (vb + 1), 1 do
                            if eb >= 0 then
                                for lb = 0, _slopeAmountCycle - (vb + 1), 1 do cWallMirrorEx(_curSide +
                                    (_spiralPosistionOffset - (_direction * _posSpacing)) +
                                    (_direction * _posSpacing * lb), _mirrorStep, _extraWidth,
                                    customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult *
                                        _sizeMult, p_getDelayPatternBool())); end
                            end
                        end
                        cWallMirrorEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) +
                            (_direction * _posSpacing * (_slopeAmountCycle - vb)), _mirrorStep, _extraWidth,
                            customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult *
                            _sizeMult, p_getDelayPatternBool()));
                    end
                    ...
                    for hb = 0, _timesBeforeChangeDir, 1 do
                        cWallMirrorEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) +
                            (_direction * _posSpacing * _slopeAmountCycle), _mirrorStep, _extraWidth,
                            customizePatternThickness((_curSpiralThick + (hb < _timesBeforeChangeDir and 1 or 0)) *
                                _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult *
                            _sizeMult, p_getDelayPatternBool()));
                    end
                end
                _spiralPosistionOffset = _spiralPosistionOffset - (_direction * _posSpacing * 2);
                _direction = _direction * -1; -- It's time to reversing the spiral.
            end
        end
        if (_slopeAmountEnd > 0) then
            for fb = 0, _slopeAmountEnd, 1 do
                for ib = 0, fb, 1 do cWallMirrorEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) +
                    (_direction * _posSpacing * ib), _mirrorStep, _extraWidth,
                    customizePatternThickness((_curSpiralThick + (fb < _slopeAmountEnd and 1 or 0)) * _curDelaySpeed *
                        _delMult * _sizeMult, p_getDelayPatternBool())); end
                t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult,
                    p_getDelayPatternBool()));
            end
        end
...
function pMarch31osTunnel(_side, _corridorThickOfSpeedLessThan, _corridorThickOfSpeedGEThan, _iter, _largeWalls, _delMult
                          , _sizeMult, _direction, _dirType, _largeWallDesign, _isDistanceTight, _isLargeWallOnce,
                          _sidedir0gap, _sidedir1gap, _sidedir0offset, _sidedir1offset, _gearTeethSizeMult, _gearTeethInc
                          , _gearTeethStepDel, _gearTeethStepLimit, _isBeforeGearTeethBegin, _isAfterGearTeethEnd,
                          _repeatCorridorAmount, _repeatCorridorDelay, _isRepeatCorridorDelaySpd, _skipEndDelay,
                          _isRebootingSide, _endAdditionalDelay, _addMult, _delayMultOfSpdLessThan,
                          _spdIsGreaterThanEqual)
    ...
    _sizeMult = anythingButNil(_sizeMult, 1);
    _dirType = anythingButNil(_dirType, 0);
    _largeWalls = closeValue(anythingButNil(_largeWalls, u_rndInt(1, getProtocolSides() - 2)), 1, getProtocolSides() - 2);
    _isDistanceTight = anythingButNil(_isDistanceTight, 0);
    ...
    _sidedir0gap = anythingButNil(_sidedir0gap, 1);
    _sidedir1gap = anythingButNil(_sidedir1gap, _sidedir0gap);
    _sidedir0offset = anythingButNil(_sidedir0offset, 0);
    _sidedir1offset = anythingButNil(_sidedir1offset, _sidedir0offset);
    ...
    local _curTunnelCorridorDirGap, _curTunnelCorridorDirOffset = 0, 0;
    local _curTunnelCorridorDir, _curTunnelSpiralCorridorDir = 1, 1;
    ...
    p_resetPatternDelaySettings();
    p_adjustPatternDelaySettings(_spdIsGreaterThanEqual or 2, _delayMultOfSpdLessThan or 1,
        _corridorThickOfSpeedLessThan or THICKNESS, _corridorThickOfSpeedGEThan);
    ...
    local _amountOfDesignType0, _tunnelSpiralExpected, _largeWallOnceExpected = 1, _dirType == 1,
        (getBooleanNumber(_isLargeWallOnce)) and (_dirType == 0 or _dirType == 2);

    _gearTeethInc = anythingButNil(_gearTeethInc, p_getPatternThickness() * (6 / getProtocolSides()));

    local _curTunnelDel = getBooleanNumber(_isDistanceTight) and
        (
        closeValue(getProtocolSides(), 3, 9) / (_largeWalls * 0.4 + 0.175) +
            (
            (p_getPatternThickness() / 2) * (_largeWalls / (getProtocolSides() - 2)) /
                customizePatternThickness(closeValue(getProtocolSides(), 3, 9), true))) or 9;
    ...
    if (_largeWallOnceExpected) then
        _constructDesignCWallExPart(_curSide + 1, _largeWallDesign, _largeWalls - 1,
            customizePatternThickness((
                _curTunnelDel * (_iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd)) * _curDelaySpeed *
    ...
    for a = 0, _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd, 1 do
        ...
        if _tunnelLoopDir > 0 then _curTunnelCorridorDirGap = _sidedir1gap;
        _curTunnelCorridorDirOffset = _sidedir1offset;
        _curTunnelCorridorDir = 1 * _curTunnelSpiralCorridorDir;

        else _curTunnelCorridorDirGap = _sidedir0gap;
        _curTunnelCorridorDirOffset = _sidedir0offset;
        _curTunnelCorridorDir = -1 * _curTunnelSpiralCorridorDir;

        end

        for repeatAmount = 0, _repeatCorridorAmount, 1 do
            if repeatAmount < _repeatCorridorAmount and (not _largeWallOnceExpected) then _constructDesignCWallExPart(_curSide
                + 1, _largeWallDesign, _largeWalls - 1,
                customizePatternThickness(_repeatCorridorDelay * _sizeMult, _isRepeatCorridorDelaySpd) +
                ((p_getPatternThickness() / 2) * _sizeMult)); end

            _gearTeethBarrageAmount = 1;
            _gearTeethBarrageStep = 0;
            _gearTeethBarrageInc = 0;
            ...
            for corridorGap = _curTunnelCorridorDirGap, getBarrageSide(_curTunnelCorridorDirOffset + _amountOfDesignType0), 1 do
                if _gearTeethSizeMult > 0 then
                    cWall((corridorGap * _curTunnelCorridorDir) + _curSide + _tunnelLoopDir +
                        (_curTunnelCorridorDirOffset * _curTunnelCorridorDir),
                        (p_getPatternThickness() + _gearTeethBarrageInc) * _gearTeethSizeMult)
                    if corridorGap < getBarrageSide(_curTunnelCorridorDirOffset + _gearTeethStepLimit) - _largeWalls then _gearTeethBarrageStep = _gearTeethBarrageStep
                            + 1;
                        if _gearTeethBarrageStep >= _gearTeethStepDel then _gearTeethBarrageStep = 0;
                        _gearTeethBarrageInc = (_gearTeethBarrageAmount * _gearTeethInc);
                        _gearTeethBarrageAmount = _gearTeethBarrageAmount + 1;
                        end
                    end
                else cWall((corridorGap * _curTunnelCorridorDir) + _curSide + _tunnelLoopDir +
                    (_curTunnelCorridorDirOffset * _curTunnelCorridorDir),
                    p_getPatternThickness() * _saveOldGearTeethSizeMult * _sizeMult)
                end
            end

            _gearTeethWallOffsetAmount = 1;
            _gearTeethWallOffsetStep = 0;
            _gearTeethWallOffsetInc = 0;

            for offsetGap = getProtocolSides() - _curTunnelCorridorDirOffset + _amountOfDesignType0, getProtocolSides(), 1 do
                if _gearTeethSizeMult > 0 then
                    cWall((offsetGap * _curTunnelCorridorDir * -1) + _curSide + _tunnelLoopDir +
                        (_curTunnelCorridorDirOffset * _curTunnelCorridorDir) -
                        (_curTunnelCorridorDir * (getProtocolSides() + _curTunnelCorridorDirOffset)),
                        (p_getPatternThickness() + _gearTeethWallOffsetInc) * _gearTeethSizeMult);
                    if offsetGap < getProtocolSides() - (_gearTeethStepLimit - 1) then _gearTeethWallOffsetStep = _gearTeethWallOffsetStep
                            + 1;
                        if _gearTeethWallOffsetStep >= _gearTeethStepDel then _gearTeethWallOffsetStep = 0;
                        _gearTeethWallOffsetInc = (_gearTeethWallOffsetAmount * _gearTeethInc);
                        _gearTeethWallOffsetAmount = _gearTeethWallOffsetAmount + 1;
                        end
                    end
                else cWall((offsetGap * _curTunnelCorridorDir) + _curSide + _tunnelLoopDir +
                    (_curTunnelCorridorDirOffset * _curTunnelCorridorDir) - _curTunnelCorridorDir,
                    p_getPatternThickness() * _saveOldGearTeethSizeMult * _sizeMult);
                end
            end
        ...
        if a < _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd and (not _largeWallOnceExpected) then
            _constructDesignCWallExPart(_curSide + 1, _largeWallDesign, _largeWalls - 1,
                customizePatternThickness(_curTunnelDel * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())
                +
                customizePatternThickness(_repeatCorridorDelay * (_repeatCorridorAmount) * _sizeMult *
                    (
                    (
                        (_dirType == 1 or _dirType >= 3) and
                            a < _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd - 1 and 1) or 0),
                    _isRepeatCorridorDelaySpd) + ((p_getPatternThickness() / 2) * _sizeMult));
        end

        if _dirType == 1 then _tunnelSpiralExpected = true;
            if a < _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd - 1 then _curSide = _curSide -
                ((_largeWalls + 1) + (_curTunnelCorridorDirGap - 1) + _curTunnelCorridorDirOffset) *
                _curTunnelCorridorDir;
        ...
        elseif _dirType == 3 then
            if a < _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd - 1 then
                if u_rndInt(0, 100) > 50 then _tunnelSpiralExpected = true;
                _curSide = _curSide -
                    ((_largeWalls + 1) + (_curTunnelCorridorDirGap - 1) + _curTunnelCorridorDirOffset) *
                    _curTunnelCorridorDir;
                _oldCurSide = _curSide
                if a == _iter + _timesOfBeforeGearTeethBegin - 1 then _curSide = _curSide -
                    (_curTunnelCorridorDirGap - 1) + _curTunnelCorridorDirOffset * _curTunnelCorridorDir;
                _curTunnelSpiralCorridorDir = _curTunnelSpiralCorridorDir * -1;
                end
                else _tunnelSpiralExpected = false;
                _tunnelLoopDir = _tunnelLoopDir * -1 + (_largeWalls + 1);
        ...
        if a < _iter + _timesOfBeforeGearTeethBegin + _timesOfAfterGearTeethEnd then t_applyPatDel(customizePatternDelay(_curTunnelDel
            * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())); end
    end

    t_applyPatDel((_endAdditionalDelay or march31oPatDel_AdditionalDelay or 0) +
        (getPerfectDelay(THICKNESS) * (getBooleanNumber(_skipEndDelay) and 8 or 12)) +
        (customizePatternDelay(0.5, false) * (_largeWalls / (getProtocolSides() - 2))));
end
