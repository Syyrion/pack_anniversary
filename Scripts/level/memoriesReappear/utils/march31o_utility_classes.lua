...
function TimerFrameTime:setTimescale(_timescale) self.timescale = type(_timescale) == 'number' and
    math.max(_timescale, 0) or 1 end
...
-- <_timescale> timescale
function PulseDetector:new(_dur_start, _launch_time, _dur_input, _is_using_bpm, _rev_del, _function_ETL, _function_OETL,
                           _loop_amount_start, _is_sync_to_dm, _timescale)
    local newInst = {}
    setmetatable(newInst, self)
...
function PulseDetector:setFunctionETL(_function_ETL) self.functionETL = (
    type(_function_ETL) == 'function' and _function_ETL) or function(...) end end

function PulseDetector:getFunctionETL() return self.functionETL end

function PulseDetector:setFunctionOETL(_function_OETL) self.functionOETL = (
    type(_function_OETL) == 'function' and _function_OETL) or function(...) end end
...
function PulseDetector:step(_timer_input, ...)
    ...
    local _usingBPMState = (self.boolOfUsingBPM == true and convertBPMtoSeconds(self.durationInput)) or
        self.durationInput;
...
function PulseDetector:resetTimer(_is_full_reset, _timer_input)
    ...
    self.launchEventTime = _timer_input or l_getLevelTime();
end
...
function EventTimer:step(_timer_input, ...)
    ...
    self.currentEventTime = (
        (_timer_input - self.launchEventTime + self.eventDurationOffset) * _syncToDMState * self.timescale)
end
    ...
    if (self.currentEventTime >= _duration_target and self.eventSection == _section_target) then
        if getBooleanNumber(_is_halt_advance) == false then self.eventSection = (self.eventSection + 1);
            ...
            if (self.isDebounceInAdvanceEventHalter) and (not self.isDebounce) then
                self.isDebounceInAdvanceEventHalter = false;
...
function EventTimer:advanceIfHalted(_duration_target)
    ...
    if (self.currentEventTime >= _duration_target) and (self.haltAdvance) then
        ...
        self.haltAdvance = false;
        self.isDebounceInAdvanceEventHalter = true;
        return true;
    end
    return false;
end
