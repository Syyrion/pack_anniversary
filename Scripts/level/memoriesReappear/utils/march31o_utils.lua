...
function getPolySides(_side_amount, _float_type)
    if type(_float_type) == "string" then _float_type = (_float_type == "floor" and math.floor) or
        (_float_type == "ceil" and math.ceil) or math.ceil; else _float_type = math.ceil; end
    return _float_type(getProtocolSides() / (type(_side_amount) == "number" and math.floor(_side_amount) or 2));
end

function getFakeSideLength(_fake_sides_amount) return math.floor(l_getSides() /
    (type(_fake_sides_amount) == "number" and math.floor(_fake_sides_amount) or 6)); end
...
function math_sum(_table_of_numbers)
    ...
    if #_table_of_numbers > 0 then
        for a = 1, #_table_of_numbers, 1 do
            if type(_table_of_numbers[a]) ~= "number" then errorf(2, "Sum",
                "table #%d is not a number. why did you input a non-number value?", a) end
...
function p_resetPatternDelaySettings()
    pstr_currentResultBool = false;
    pstr_currentDelayMultOfSpeedLessThan = 0;
    pstr_currentThickness = THICKNESS;
end

function p_adjustPatternDelaySettings(_spdMultDMIsGreaterThanEqual, _delMultOfSpeedLessThan, _corridorThickOfSpdLessThan
                                      , _corridorThickOfSpdGreaterThanEqual)
    if u_getSpeedMultDM() < _spdMultDMIsGreaterThanEqual then pstr_currentDelayMultOfSpeedLessThan = _delMultOfSpeedLessThan;
    pstr_currentResultBool = false;
    pstr_currentThickness = _corridorThickOfSpdLessThan or THICKNESS;

    elseif u_getSpeedMultDM() >= _spdMultDMIsGreaterThanEqual then pstr_currentDelayMultOfSpeedLessThan = 1;
    pstr_currentResultBool = true;
    pstr_currentThickness = _corridorThickOfSpdGreaterThanEqual or _corridorThickOfSpdLessThan;

    end
end

function p_setOverrideShape(_sideType, _emulatedSides)
    if type(_sideType) ~= "number" then errorf(2, "SideType",
        "argument #1 is not a number. why did you input a non-number value?") end
    if type(_emulatedSides) ~= "number" then errorf(2, "EmluateSide",
        "argument #2 is not a number. why did you input a non-number value?") end
    SHAPE_TYPE = math.floor(_sideType) or 0;
...
function l_updateLevelFrameTime(mFrameTime, _fps) marchiocommon_currentTimerAmount = marchiocommon_currentTimerAmount +
    (mFrameTime / (_fps or 60)); end
...
function e_detectEventSection(_duration_target, _section_target, _is_halt_advance)
    if (marchiocommon_currentEventTime >= _duration_target and marchiocommon_currentEventSection == _section_target) then
        if getBooleanNumber(_is_halt_advance) == false then marchiocommon_currentEventSection = (
            marchiocommon_currentEventSection + 1);
        else
            marchiocommon_isAdvanceHalted = true;
            if (marchiocommon_isDebounceInAdvanceEventHalter) and (not marchiocommon_isDebounce) then
                marchiocommon_isDebounceInAdvanceEventHalter = false;
                marchiocommon_isDebounce = true;
            end
        end
        return true;
    end
...
function e_advanceEventSectionIfHalted(_duration_target)
    ...
    if (marchiocommon_currentEventTime >= _duration_target) and (marchiocommon_isAdvanceHalted) then
        ...
        marchiocommon_isAdvanceHalted = false;
        marchiocommon_isDebounceInAdvanceEventHalter = true;
        return true;
    end
    return false;
end
...
--delays
function getDelayWalls(_delay_amount) return (
    (THICKNESS - THICKNESS_PERFECT_ADD_ENDED) / (5.02 * u_getSpeedMultDM()) * u_getDelayMultDM()) * _delay_amount; end
...
--thickness
function getAccurateThickness() return ((360 / getProtocolSides()) / PLAYER_SPEED) / u_getDelayMultDM() *
    (5.02 * u_getSpeedMultDM()) + THICKNESS_PERFECT_ADD_ENDED; end

function getThicknessOfDelay(_delay_amount) return _delay_amount * 5.02 * u_getSpeedMultDM() * u_getDelayMultDM() +
    THICKNESS_PERFECT_ADD_ENDED; end
...
function getPulseContribution(_game_ver)
    ...
    else return getLevelValueFloat("beatpulse_max") +
        getLevelValueFloat("radius_min") * (getLevelValueFloat("pulse_max") / getLevelValueFloat("pulse_min") - 1)
    end
end
...
function getRebootPatternHalfSide(_side_div) return u_rndInt(1,
    math.floor(getProtocolSides() / (type(_side_div) == "number" and _side_div or 2)) - 1); end -- u_rndInt(1, halfsides)

function getPolygonExcept(_side, _side_div) return math.floor((
    getProtocolSides() - (type(_side) == "number" and _side or 6)) / (type(_side_div) == "number" and _side_div or 2)); end -- non-polygon boost
...
function getRotationDir(_game_ver)
    ...
    else return (getLevelValueFloat("rotation_speed") > 0 and 1) or (getLevelValueFloat("rotation_speed") < 0 and -1) or
        0
    end
end

--utils
function convValue(mFrameTime, _value, _valueTo, _strength) return _value -
    (0.1 * _strength * (_value - _valueTo)) * mFrameTime end
...
function quantizeValue(_val, _div_primary, _div_secondary) return math.floor(_val * _div_primary) /
    (_div_secondary or _div_primary); end
...
function u_forceManualIncrementV1Compatibility(_rot_inc, _spd_inc, _del_inc)
    ...
    _spd_inc = type(_spd_inc) == "number" and _spd_inc or 0;
    _del_inc = type(_del_inc) == "number" and _del_inc or 0;
    setLevelValueFloat("rotation_speed",
        math_add(getLevelValueFloat("rotation_speed"), _rot_inc * getDir(getLevelValueFloat("rotation_speed"))) * -1);
...
function u_setManualIncrementDurationOffset(_offset_amount) marchiocommon_currentManualIncrementTimer = _offset_amount or
    0; end
...
--syncs
function convertBPMtoSeconds(_tempo, _division, _multiply) return (((_tempo / 60) ^ -1) / (_division or 1)) *
    (_multiply or 1); end
...
function l_setSyncedPulse(mFrameTime, _tempo_input, _is_advanced_tempo, _effect_type, _is_instant_pul_spd, _lvl_pul_min,
                          _lvl_pul_max, _lvl_pul_spd, _lvl_pul_spd_r, _lvl_pul_dir, _lvl_beat_pul_max, _lvl_beat_pul_spd
                          , _lvl_rot_spd, _lvl_rot_spd_mult, _lvl_rot_spd_dir, _rev_del, _is_sync_to_dm, _timer_input)
    ...
    local marchiocommon_DMvalue = (getBooleanNumber(_is_sync_to_dm) == true and u_getDifficultyMult() ^ 0.12) or 1;
    local marchiocommon_tempoState = (getBooleanNumber(_is_advanced_tempo) == true and convertBPMtoSeconds(_tempo_input)
        ) or _tempo_input;
    ...
    if type(_lvl_pul_spd) ~= "number" then errorf(2, "SyncPulse",
        "argument #8 is not a number. why did you input a non-number value?") end
    if type(_lvl_pul_spd_r) ~= "number" then errorf(2, "SyncPulse",
        "argument #9 is literally not a number. why did you input a non-number value?") end
    if type(_lvl_pul_dir) ~= "number" then errorf(2, "SyncPulse",
        "argument #10 is LITERALLY LITERALLY not a number. why did you input a non-number value?") end
    ...
    if (_lvl_beat_pul_max ~= 0 and type(_lvl_beat_pul_max) == "number") then
        marchiocommon_currentBeatPulse = closeValue(marchiocommon_currentBeatPulse - (_lvl_beat_pul_spd * mFrameTime * 2
            ), 0, 9999, "min");
    ...
    if ((_timer_input * marchiocommon_DMvalue) > marchiocommon_currentSyncedPulseTimerOffsetFix) then
        ...
        if (
            (_timer_input * marchiocommon_DMvalue) <
                marchiocommon_currentSyncedPulseTimer - marchiocommon_tempoState + _rev_del) then
            ...
            if _effect_type == 1 or _effect_type >= 256 then
...
function l_setSyncedPulseInstant(mFrameTime, _tempo_input, _is_advanced_tempo, _effect_type, _lvl_pul_min, _lvl_pul_max,
                                 _lvl_pul_spd, _lvl_pul_spd_r, _lvl_pul_dir, _lvl_beat_pul_max, _lvl_beat_pul_spd,
                                 _lvl_rot_spd, _lvl_rot_spd_mult, _lvl_rot_spd_dir, _rev_del, _is_sync_to_dm,
                                 _timer_input)
    if _is_advanced_tempo == nil then _is_advanced_tempo = false; end
    if _lvl_rot_spd_mult == nil then _lvl_rot_spd_mult = 1; end
    if _lvl_beat_pul_spd == nil then _lvl_beat_pul_spd = 1; end
    ...
    if type(_lvl_pul_dir) ~= "number" then errorf(2, "SyncPulse",
        "argument #9 is LITERALLY LITERALLY not a number. why did you input a non-number value?") end
    ...
    if ((_timer_input * marchiocommon_DMvalue) > marchiocommon_currentSyncedPulseTimerOffsetFix) then
        ...
        if (
            ...
            if _effect_type == 1 or _effect_type >= 256 then l_setRotationSpeed(_lvl_rot_spd * _lvl_rot_spd_mult *
                _lvl_rot_spd_dir) end
        end
...
function l_setSyncedPulseV1Compatibility(mFrameTime, _tempo_input, _is_advanced_tempo, _effect_type, _is_instant_pul_spd
                                         , _lvl_pul_min, _lvl_pul_max, _lvl_pul_spd, _lvl_pul_spd_r, _lvl_pul_dir,
                                         _lvl_beat_pul_max, _lvl_beat_pul_spd, _lvl_rot_spd, _lvl_rot_spd_mult,
                                         _lvl_rot_spd_dir, _rev_del, _is_sync_to_dm, _timer_input)
    if _is_advanced_tempo == nil then _is_advanced_tempo = false; end
    if _lvl_rot_spd_mult == nil then _lvl_rot_spd_mult = 1; end
    if _lvl_beat_pul_spd == nil then _lvl_beat_pul_spd = 1; end
    if _lvl_rot_spd_dir == nil or _lvl_rot_spd_dir == 0 or _lvl_rot_spd_dir > 1 or _lvl_rot_spd_dir < -1 then _lvl_rot_spd_dir = 1; end
    ...
    if ((_timer_input * marchiocommon_DMvalue) > marchiocommon_currentSyncedPulseTimerOffsetFix) then
        ...
        if (
            (_timer_input * marchiocommon_DMvalue) <
                marchiocommon_currentSyncedPulseTimer - marchiocommon_tempoState + _rev_del) then
            if _effect_type == 0 or _effect_type >= 256 then
                if _lvl_pul_dir > 0 and _lvl_pul_spd > 0 then setLevelValueFloat("pulse_speed", _lvl_pul_spd); setLevelValueFloat("pulse_speed_r"
                    , marchiocommon_instPulSpdState);
                elseif _lvl_pul_dir < 0 and _lvl_pul_spd_r > 0 then setLevelValueFloat("pulse_speed",
                    marchiocommon_instPulSpdRState); setLevelValueFloat("pulse_speed_r", _lvl_pul_spd_r);
                end
            end
            if _effect_type == 1 or _effect_type >= 256 then
        ...
        if (
            ...
            if _effect_type == 0 or _effect_type >= 256 then
                if _lvl_pul_dir > 0 and _lvl_pul_spd_r > 0 then setLevelValueFloat("pulse_speed", 0); setLevelValueFloat("pulse_speed_r"
                    , _lvl_pul_spd_r);
                elseif _lvl_pul_dir < 0 and _lvl_pul_spd > 0 then setLevelValueFloat("pulse_speed", _lvl_pul_spd); setLevelValueFloat("pulse_speed_r"
                    , 0);
                end
            end
            if _effect_type == 1 or _effect_type >= 256 then
...
function l_setPulseDetector(_dur_input, _is_using_bpm, _rev_del, _is_sync_to_dm, _timer_input)
    ...
    local marchiocommon_DMvalue = (getBooleanNumber(_is_sync_to_dm) == true and u_getDifficultyMult() ^ 0.12) or 1;
    local marchiocommon_tempoState = (getBooleanNumber(_is_using_bpm) == true and convertBPMtoSeconds(_dur_input)) or
        _dur_input;
    ...
    if (
        (_timer_input * marchiocommon_DMvalue) >
            marchiocommon_currentPulseDetectorTimer - marchiocommon_tempoState + _rev_del) then
...
function customizePatternDelay(_delay_amount, _is_spd_mult)
    ...
    return (
        getPerfectDelay(THICKNESS) *
            (_delay_amount * marchiocommon_currentOverridePatternDelay * pstr_currentDelayMultOfSpeedLessThan) *
            (getBooleanNumber(_is_spd_mult) and (u_getSpeedMultDM()) / 2 or 1));
end

function customizePatternThickness(_thickness_amount, _is_spd_mult)
    if _is_spd_mult == nil then _is_spd_mult = true; end
    return (
        (THICKNESS * u_getDelayMultDM()) *
            (_thickness_amount * marchiocommon_currentOverridePatternDelay * pstr_currentDelayMultOfSpeedLessThan) *
            (getBooleanNumber(_is_spd_mult) and (u_getSpeedMultDM()) / 2 or 1));
end
...
function customizeTempoPatternThickness(_thickness_amount)
    return (
        convertBPMtoSeconds(GLOBAL_TEMPO * GLOBAL_TEMPO_DM_STATE) * _thickness_amount * 3.781 * 80 * u_getSpeedMultDM())
        + 1;
end

--patterns
function p_updateGlobalSyncedPatterns()
    t_eval([[l_setWallSpawnDistance(]] ..
        70 +
        1100 * u_getSpeedMultDM() * 1.075 * convertBPMtoSeconds(GLOBAL_TEMPO * GLOBAL_TEMPO_DM_STATE) *
        GLOBAL_SPAWN_DISTANCE_MULT * GLOBAL_TIME_SIGNATURE + GLOBAL_SPAWN_DISTANCE_ADD .. [[)]])
end

local marchiocommon_oldSyncedPatternRepairSeconds = 0;

function p_setGlobalSyncedPatternSettings(_tempo, _spawn_dist_mult, _spawn_dist_add, _1st_time_signature,
                                          _2nd_time_signature, _is_sync_to_dm)
    GLOBAL_TEMPO = _tempo or 128;
    GLOBAL_SPAWN_DISTANCE_MULT = _spawn_dist_mult or 1;
    GLOBAL_SPAWN_DISTANCE_ADD = _spawn_dist_add or 0;
    GLOBAL_TIME_SIGNATURE = (type(_1st_time_signature) == "number" and _1st_time_signature or 4) /
        (type(_2nd_time_signature) == "number" and _2nd_time_signature or 4);
    GLOBAL_TEMPO_DM_STATE = (getBooleanNumber(_is_sync_to_dm) == true and u_getDifficultyMult() ^ 0.12) or 1;
    marchiocommon_oldSyncedPatternRepairSeconds = (
        70 +
            1100 * u_getSpeedMultDM() * 1.075 * convertBPMtoSeconds(GLOBAL_TEMPO) * GLOBAL_SPAWN_DISTANCE_MULT *
            GLOBAL_TIME_SIGNATURE + GLOBAL_SPAWN_DISTANCE_ADD) / (70 + 1100);
end
...
function p_updateSyncedPatternRepair(_beat_duration_amount, _beat_delay_amount, _is_spd_inc_fix, _func_primary,
                                     _func_secondary, _timer_input)
    _timer_input = _timer_input or l_getLevelTime();
    _func_primary = _func_primary or t_clear;
    local marchiocommon_currentSyncedPatternRepairSeconds = (
        70 + 1100 * u_getSpeedMultDM() * 1.075 * convertBPMtoSeconds(GLOBAL_TEMPO) * GLOBAL_TIME_SIGNATURE) / (70 + 1100
        );
    local _spd_inc_fix_state = marchiocommon_currentSyncedPatternRepairSeconds -
        marchiocommon_oldSyncedPatternRepairSeconds
    local _spd_inc_timer_diff_greater_fix_state = (u_getDifficultyMult() >= 1 and (u_getDifficultyMult() ^ -0.25) * 0.65
        ) or 0.65;
    ...
    if getBooleanNumber(_is_spd_inc_fix) then
        if (
            (_timer_input * GLOBAL_TEMPO_DM_STATE) -
                ((u_getDifficultyMult() ^ _spd_inc_timer_diff_greater_fix_state) - 1) +
                _spd_inc_timer_diff_less_fix_state) >
            marchiocommon_currentSyncedPatternRepairDuration +
            (_beat_duration_amount * convertBPMtoSeconds(GLOBAL_TEMPO)) +
            (_beat_delay_amount * convertBPMtoSeconds(GLOBAL_TEMPO)) + _spd_inc_fix_state then
            marchiocommon_currentSyncedPatternRepairDuration = marchiocommon_currentSyncedPatternRepairDuration +
                (_beat_duration_amount * convertBPMtoSeconds(GLOBAL_TEMPO));
        ...
        if (_timer_input * GLOBAL_TEMPO_DM_STATE) >
            marchiocommon_currentSyncedPatternRepairDuration +
            (_beat_duration_amount * convertBPMtoSeconds(GLOBAL_TEMPO)) +
            (_beat_delay_amount * convertBPMtoSeconds(GLOBAL_TEMPO)) then
            marchiocommon_currentSyncedPatternRepairDuration = marchiocommon_currentSyncedPatternRepairDuration +
                (_beat_duration_amount * convertBPMtoSeconds(GLOBAL_TEMPO));
            marchiocommon_currentSyncedPatternRepairTimes = math_add(marchiocommon_currentSyncedPatternRepairTimes, 1);
            _func_primary();
...
function getRandomNegVal(_input)
    if type(_input) ~= "number" then errorf(2, "Input",
        "argument #1 is not a number. why did you input a non-number value?") end
...
function u_updateRotSpdOfFastSpinning(mFrameTime, _conv_mult) marchiocommon_currentRotSpdMultOfFastSpin = closeValue(marchiocommon_currentRotSpdMultOfFastSpin
    - ((_conv_mult or 1) * mFrameTime), 1, 9999, "min"); end
...
function l_setSyncedBeatPulse(mFrameTime, _game_ver, _tempo_input, _is_advanced_tempo, _lvl_beat_pul_max,
                              _lvl_beat_pul_spd, _is_sync_to_dm, _timer_input)
    _game_ver = _game_ver or game_version
    if not _is_using_bpm then _is_using_bpm = false; end
    if not _is_sync_to_dm then _is_sync_to_dm = false; end
    _timer_input = _timer_input or l_getLevelTime();
    ...
    if ((_timer_input * marchiocommon_DMvalue) > marchiocommon_currentSyncedBeatPulseTimer) then
        marchiocommon_currentSyncedBeatPulseTimer = (
            marchiocommon_currentSyncedBeatPulseTimer + marchiocommon_tempoState);
    ...
    marchiocommon_currentSyncedBeatPulseAmount = closeValue(marchiocommon_currentSyncedBeatPulseAmount -
        (_lvl_beat_pul_spd * mFrameTime * 2), 0, 9999, "min");
    if _game_ver >= 2 then
...
function l_setSyncedAdvancedPulse(mFrameTime, _game_ver, _tempo_input, _is_advanced_tempo, _pul_override_amount,
                                  _lvl_pul_min, _lvl_pul_max, _lvl_pul_spd, _lvl_pul_spd_r, _lvl_pul_dir, _pul_rev_del,
                                  _lvl_beat_pul_max, _lvl_beat_pul_spd, _lvl_rad_min, _is_sync_to_dm, _timer_input)
    _game_ver = _game_ver or game_version
    if _is_advanced_tempo == nil then _is_advanced_tempo = false; end
    if _is_sync_to_dm == nil then _is_sync_to_dm = false; end
    _timer_input = _timer_input or l_getLevelTime();
    ...
    if (
        (_timer_input * marchiocommon_DMvalue) >
            marchiocommon_currentSyncedAdvPulseTimer - marchiocommon_tempoState + _pul_rev_del) then
    ...
    marchiocommon_currentAdvPulse = closeValue(marchiocommon_currentAdvPulse +
        (marchiocommon_currentAdvPulseSpd * mFrameTime), _lvl_pul_min, _lvl_pul_max, "all");

    if (_lvl_beat_pul_max ~= 0 and type(_lvl_beat_pul_max) == "number") then
        marchiocommon_currentAdvBeatPulse = closeValue(marchiocommon_currentAdvBeatPulse -
            (_lvl_beat_pul_spd * mFrameTime * 2), 0, 9999, "min");
    end

    if _game_ver >= 2 then
        l_setPulseMin((_lvl_rad_min or 70) + (_pul_override_amount or 0) +
            (marchiocommon_currentAdvPulse * (((math.pi * 0.5) - math.pi) ^ -1) * ((_lvl_rad_min or 70) / 70)));
        ...
        setLevelValueFloat("pulse_min",
            (_lvl_rad_min or 70) + (_pul_override_amount or 0) +
            (marchiocommon_currentAdvPulse * (((math.pi * 0.5) - math.pi) ^ -1) * ((_lvl_rad_min or 70) / 70)));
        setLevelValueFloat("radius_min", _lvl_rad_min or 70);
        if (_lvl_beat_pul_max ~= 0 and type(_lvl_beat_pul_max) == "number") then setLevelValueFloat("beatpulse_max",
            marchiocommon_currentAdvBeatPulse); end
    end
end
...
function l_setSyncedAdvancedPulseTimerOffset(_offset_amount) marchiocommon_currentSyncedAdvPulseTimer = _offset_amount or
    0; end
