...
void main (void) {
    ...
    st = tile(st,3.0);
	// basically a tiling of 6x, but each is rotated a different direction
    st = rotateTilePattern(st); 

    st = tile(st,1.5); // tiling it by 1.5 makes it look REALLY cool
     //st = rotate2D(st,-PI*u_time*0.25);
     //st = rotateTilePattern(st*1.1);
     st = rotate2D(st, u_time);

    gl_FragColor = vec4(mix(color1, color2, st.y),1.0);
}
