...
void main (void) {
    ...
    st = rotateTilePattern(st * 1.8); // some crazy shit
    st = rotateTilePattern(st * 0.71);
    st = rotate2D(st, u_time * 3.0);

    gl_FragColor = vec4(mix(color1, color2, step(0.8, length(st))),1.0);
}
